...
Future<bool> _isValidServer(String serverUrl) async {
  try {
    ...
    if (_jsonResponse.containsKey("status") &&
        _jsonResponse['status'].toString().toLowerCase() == "true") {
      ...
      throw new ErrorDescription(
          "email-auth >> Server returned the following data :: $_jsonResponse");
    }
...
class EmailAuth {
    ...
    try {
      ...
      if (data.containsKey('server') &&
        ...
        if (data['server']![data['server']!.length - 1] == "/") {
          data['server'] =
              data['server']!.substring(0, data['server']!.length - 1);
        }
  ...
  Future<bool> sendOtp(
    ...
    try {
      ...
      if (_serverRuntime["validRemote"]!.toLowerCase() == "false" ||
          _serverRuntime["server"]!.length <= 0) {
        print(
            "email-auth >> Remote server is not available -- using test server --");
        print("email-auth >> ❗ Warning this is not reliable on production");
        print(
            "email-auth >> Test servers are marked obselete, Kindly set up a production server");
        return false;
      ...
      } else if (_serverRuntime["validRemote"] != null &&
          _serverRuntime["validRemote"]!.toLowerCase() == "true") {
        http.Response _response = await http.get(Uri.parse(
